...
if ( ! function_exists( 'uxb_team_init_plugin' ) ) {
	...
	function uxb_team_init_plugin() {
		...
		if ( is_plugin_active( 'option-tree/ot-loader.php' ) ) {
			...
			add_action( 'admin_init', 'uxb_team_create_team_excerpt_setting' );
			add_action( 'admin_init', 'uxb_team_create_team_single_page_image_setting' );
			add_action( 'admin_init', 'uxb_team_create_team_meta_info' );
			add_action( 'admin_init', 'uxb_team_create_team_social_network_setting' );
		...
		if ( is_plugin_active( 'js_composer/js_composer.php' ) ) {
			...
			add_action( 'vc_before_init', 'uxb_team_load_team_member_element', 11 );
			add_action( 'init', 'uxb_team_update_element_params', 11 );
