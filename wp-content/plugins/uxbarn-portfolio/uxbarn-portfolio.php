...
define( 'UXB_PORT_CUSTOM_META_ID_INITIAL', 'uxb_port_po_custom_meta_info_' );
...
if ( ! function_exists( 'uxb_port_init_plugin' ) ) {
	...
	function uxb_port_init_plugin() {
		...
		if ( is_plugin_active( 'option-tree/ot-loader.php' ) ) {
			...
			add_action( 'admin_init', 'uxb_port_create_alternate_content' );
			add_action( 'admin_init', 'uxb_port_create_meta_info' );
			add_action( 'admin_init', 'uxb_port_create_item_format_setting' );
			add_action( 'admin_init', 'uxb_port_create_image_slideshow_format_content' );
			add_action( 'admin_init', 'uxb_port_create_video_format_content' );
		...
		if ( is_plugin_active( 'js_composer/js_composer.php' ) ) {
			...
			add_action( 'vc_before_init', 'uxb_port_load_portfolio_element', 11 );
			add_action( 'init', 'uxb_port_update_element_params', 11 );
