...
?>
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id('title') ); ?>"><?php echo esc_html( __('Title:', 'uxb_tmnl') ); ?></label>
			<input class="widefat" id="<?php echo esc_attr( $this->get_field_id('title') ); ?>" name="<?php echo esc_attr( $this->get_field_name('title') ); ?>" type="text" value="<?php echo esc_attr($title); ?>" />
		</p>
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id('auto_rotation_duration') ); ?>"><?php echo esc_html( __('Auto rotation:', 'uxb_tmnl') ); ?></label>
            <select id="<?php echo esc_attr( $this->get_field_id('auto_rotation_duration') ); ?>" name="testimonialAutoRotation">
                <option value="0" <?php echo (esc_attr($auto_rotation_duration) == '0' ? 'selected="selected"' : ''); ?>><?php echo esc_html( __('Disable', 'uxb_tmnl') ); ?></option>
		...
		<p>
			<label for=""><?php esc_html( __('Select specific testimonials to be shown:', 'uxb_tmnl' ) ); ?></label>
			<br/>
			<em><?php esc_html( __('(*Uncheck all will show every testimonials.)', 'uxb_tmnl') ); ?></em>
			...
			<?php 
				...
				if($testimonial_list->have_posts()) : while($testimonial_list->have_posts()) : $testimonial_list->the_post();
					?>
					<input id="<?php echo esc_attr( $this->get_field_id('checked_list') . '_' . get_the_ID() ); ?>" name="TestimonialGroup[]" type="checkbox" value="<?php echo get_the_ID(); ?>" 
						<?php  
						...
						?> 
					/>
					<label for="<?php echo esc_attr( $this->get_field_id('checked_list') . '_' . get_the_ID() ); ?>"><?php the_title(); ?></label>
					<br/>
					<?php
...
<?php
	...
	public function widget($args, $instance) {
		...
		echo wp_kses( $before_widget, uxb_tmnl_get_allowed_tags_for_widgets() );
		
		if (!empty($title)) {
			echo wp_kses( $before_title . $title . $after_title, uxb_tmnl_get_allowed_tags_for_widgets() );
		}
		...
		<div class="uxb-tmnl-testimonial-wrapper style2 widget">
            ...
            <div class="uxb-tmnl-testimonial-inner">
                <div class="uxb-tmnl-testimonial-list" data-auto-rotation="<?php echo esc_attr( $auto_rotation_duration ); ?>">
		          ...
		          ?>  
    		          <div class="uxb-tmnl-testimonial-item">
                        <div class="uxb-tmnl-blockquote-wrapper">
            				<blockquote>
            				    <p>
            					   <?php echo get_post_meta(get_the_ID(), 'uxbarn_testimonial_text', true); ?>
            					</p>
            				</blockquote>
		...
		echo wp_kses( $after_widget, uxb_tmnl_get_allowed_tags_for_widgets() );
